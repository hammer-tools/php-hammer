...
class PluginUpdateService : ProjectActivity {
    ...
    private val tripleInspections =
        NotificationService.NotificationItem("inspections", "inspections", ProjectService.instance.urls.inspectionsUrl)

    private val tripleReviewNow =
        NotificationService.NotificationItem("review", "review", ProjectService.instance.urls.reviewsUrl) { notification ->
            closeReview(notification)
        }
    private val tripleReviewLater =
        NotificationService.NotificationItem("Remember later") { notification -> closeReview(notification, true) }
